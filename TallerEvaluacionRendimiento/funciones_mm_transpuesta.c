...
#include <time.h>                   // Incluye la biblioteca para manejo del tiempo, útil para inicializar la semilla de números aleatorios.
#include "funciones_MMT.h"          // Incluye un archivo de cabecera personalizado que define estructuras y prototipos de funciones.
...
void llenar_matriz(int SZ){        // Función para llenar las matrices A y B con valores y C con ceros.
	srand48(time(NULL));            // Establece la semilla para la generación de números pseudoaleatorios con la hora actual.
...
void *mult_thread(void *variables){       // Función que ejecuta cada hilo para realizar parte de la multiplicación de matrices.
    ...
    for (int i = ini; i < fin; i++){      // Bucle para recorrer las filas asignadas al hilo.
        for (int j = 0; j < N; j++){      // Bucle para recorrer todas las columnas.
			...
			pB = mB + (j*N);              // Establece pB al inicio de la fila j de B, transponiendo la matriz B.
            for (int k = 0; k < N; k++, pA++, pB++){ // Bucle para sumar el producto de los elementos correspondientes.
